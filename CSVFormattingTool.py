...
for i in range(n):
    ...
    df['ProcessDate'].replace({'-': ''}, inplace=True, regex=True)
    # In[51]:
    # Change new datetime column to datetime format
    # df['DATETIME2'] = pd.to_datetime(df['DATETIME2'], format="%d%b%y %H:%M:%S")
    df['ProcessDate'] = pd.to_datetime(df.iloc[1:len(df),len(df.columns)-1], format="%d%b%y %H:%M:%S")

    # In[53]:
    # Change datetime column format to look like 01/31/2015 0:00:00
    df['ProcessDate'] = df['ProcessDate'].dt.strftime('%m/%d/%Y %H:%M:%S')
    ...
    df = df.rename(columns={'DATE': 'ProcessDate'})
    ...
    mdf = mdf[['DATETIME', 'TAGNAME', 'VALUE']]
